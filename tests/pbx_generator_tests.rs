...
fn generator_names_source_directory_roots_under_custom_groups_like_xcodegen() {
    let temp = tempfile::TempDir::new().unwrap();
    fs::create_dir_all(temp.path().join("AppRoot/Sources/App")).unwrap();
    fs::write(temp.path().join("AppRoot/Sources/App/App.swift"), "").unwrap();
        ...
        serde_json::json!({
            ...
            "targets": {
                "App": {
                    ...
                    "platform": "iOS",
                    "sources": [
                        {"path": "AppRoot/Sources", "group": "AppRoot"}
                    ]
                }
            }
        }),
